...
void * multiply(void * arg) {
    ...
    for (int i = data -> start_idx; i < data -> end_idx; i++) {
        if (data->use_hybrid_mul) {
            hybrid_mul(data -> current_row[i], data -> prev_row[2 * i], data -> prev_row[2 * i + 1], 0, data->max_depth_val);
        } else {
...
void * mod(void * arg) {
    ...
    for(int i = data -> start_idx; i < data -> end_idx; i++){
        if (data->use_hybrid_mul) {
            // printf("DEBUG: Mod function - Number bit size: %zu\n", mpz_sizeinbase(data->current_row[i], 2));
            hybrid_mul(sq1, data -> current_row[i], data -> current_row[i], 0, data->max_depth_val);
        } else {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = line_size, count = 0, prev = 0; i >= 1; i = ceil(i / 2.0), count += 1) {
        ...
        if (i == line_size) {
            ...
            for (th = 0; th < thread_num; th++) {
                ...
                threadData[th].use_hybrid_mul = (i == 2);
                threadData[th].max_depth_val = 2;
    ...
    for (int i = row - 2; i >= 0; i--) {
        ...
        for(th = 0 ; th < thread_num ; th++){
            ...
            threadData[th].start_idx = j;
            threadData[th].end_idx = j+fin_load;
            threadData[th].use_hybrid_mul = max_depth_val > 0;
            threadData[th].max_depth_val = max_depth_val;
