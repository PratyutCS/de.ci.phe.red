...
void * mod(void * arg) {
    ...
    for(int i = data -> start_idx; i < data -> end_idx; i++){
        if (data->use_hybrid_mul) {
            ...
            int max_depth_val = 3; // Reduced for debugging
            hybrid_mul(sq1, data -> current_row[i], data -> current_row[i], 0, max_depth_val);
        } else {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = row - 2; i >= 0; i--) {
        ...
        for(th = 0 ; th < thread_num ; th++){
            ...
            threadData[th].start_idx = j;
            threadData[th].end_idx = j+fin_load;
            threadData[th].use_hybrid_mul = (i == row - 2) ? 1 : 0; // Set flag for upper layer
